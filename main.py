...
st.markdown('<h1 class="main-header">📊 测量误差与数据处理动态演示系统</h1>', unsafe_allow_html=True)
...
N_valid = len(x_valid)
x_bar = np.mean(x_i)
x_bar_valid = np.mean(x_valid) if N_valid > 0 else 0

# 残差计算
v_i = x_i - x_bar  # 全数据残差
v_i_valid = x_valid - x_bar_valid  # 有效数据残差

# 贝塞尔公式计算标准差（ddof=1 -> 除以 n-1）
if N > 1:
    sigma_bessel = np.sqrt(np.sum(v_i**2) / (N - 1))
else:
    sigma_bessel = 0

if N_valid > 1:
    sigma_bessel_valid = np.sqrt(np.sum(v_i_valid**2) / (N_valid - 1))
else:
    sigma_bessel_valid = 0
...
with tab2:
    ...
    # 执行判别
    if method == "莱特准则 (3σ)":
        threshold = 3 * sigma_bessel
        is_gross = np.abs(v_i) > threshold
        if N < 4:
            st.warning("样本量较小 (n<4)，判别结果可能不可靠")
    else:
        # 格拉布斯准则
        if N < 3:
            st.error("格拉布斯准则需要至少 3 个数据点")
            is_gross = np.zeros(N, dtype=bool)
        else:
            t_crit = stats.t.ppf(0.975, N - 2)
            G_crit = (N - 1) / np.sqrt(N) * np.sqrt(t_crit**2 / (N - 2 + t_crit**2))
            v_max = np.max(np.abs(v_i))
            G_stat = v_max / sigma_bessel if sigma_bessel > 0 else 0
            is_gross = np.abs(v_i) > G_crit * sigma_bessel
            threshold = G_crit * sigma_bessel

    gross_indices = np.where(is_gross)[0]
    ...
    fig2.add_trace(go.Scatter(
        ...
        name='测量值',
        marker=dict(
            color=['#EF4444' if is_gross[i] else '#4A90D9' for i in range(N)],
            size=12,
            symbol='circle-open'
        )
    ))
    ...
    # 标出粗大误差点（更大更醒目）
    if len(gross_indices) > 0:
        fig2.add_trace(go.Scatter(
            x=gross_indices + 1, y=x_i[gross_indices],
            mode='markers',
            name='粗大误差',
            marker=dict(color='#EF4444', size=20, symbol='x', line=dict(width=3))
        ))

    fig2.add_hline(y=x_bar, line_dash='dash', line_color='gray', annotation_text=f'x̄ = {x_bar:.2f}')
    fig2.update_layout(
        template='plotly_white',
        height=400,
        xaxis_title='测量序号 i',
    ...
    with col_a:
        st.metric("检出粗大误差点数", len(gross_indices))
        if len(gross_indices) > 0:
            st.caption(f"索引: {list(gross_indices + 1)}")
    with col_b:
        if method == "莱特准则 (3σ)":
            st.metric("判别阈值", f"3σ = {threshold:.4f}")
        else:
            st.metric("G 临界值", f"{G_crit:.4f}")

    # 剔除按钮
    st.markdown("---")
    if len(gross_indices) > 0:
        if st.button("剔除异常点并重算", type="primary"):
            new_mask = clean_mask.copy()
            new_mask[gross_indices] = False
            st.session_state['removed_mask'] = new_mask
            st.session_state['cleaned_data'] = x_i[new_mask]
            st.success(f"已剔除 {len(gross_indices)} 个粗大误差点")
            st.rerun()
    else:
